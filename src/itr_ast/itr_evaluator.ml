module Itr_ast_pp (Datetime : Itr_ast.Datetime) = struct
  ...
  module Itr_ast_pp = Itr_ast_pp.Itr_ast_pp (Datetime)
  module Itr_ast_json_pp = Itr_ast_json_pp.Itr_ast_json_pp (Datetime)
  ...
  let check_cmp lhs rhs op =
    ...
    | "<" ->
      ...
      | ( Value (Literal (Datetime (UTCTimeOnly l)))
        , Value (Literal (Datetime (UTCTimeOnly r))) ) ->
          if Datetime.utctimeonly_LessThan l r then e_true else e_false
    ...
    | ">" ->
      ...
      | ( Value (Literal (Datetime (UTCTimeOnly l)))
        , Value (Literal (Datetime (UTCTimeOnly r))) ) ->
          if Datetime.utctimeonly_GreaterThan l r then e_true else e_false
      | ( Value (Literal (Datetime (UTCDateOnly l)))
        , Value (Literal (Datetime (UTCDateOnly r))) ) ->
    ...
    | "<=" ->
      ...
      | ( Value (Literal (Datetime (UTCTimeOnly l)))
        , Value (Literal (Datetime (UTCTimeOnly r))) ) ->
          if Datetime.utctimeonly_LessThanEqual l r then e_true else e_false
      | ( Value (Literal (Datetime (UTCDateOnly l)))
        , Value (Literal (Datetime (UTCDateOnly r))) ) ->
    ...
    | ">=" ->
      ...
      | ( Value (Literal (Datetime (UTCTimeOnly l)))
        , Value (Literal (Datetime (UTCTimeOnly r))) ) ->
          if Datetime.utctimeonly_GreaterThanEqual l r then e_true else e_false
      | ( Value (Literal (Datetime (UTCDateOnly l)))
        , Value (Literal (Datetime (UTCDateOnly r))) ) ->
      ...
      | [ l; r ] ->
          ...
          | ( Rec_value (Value (Literal (Coll l)))
            , Rec_value (Value (Literal (Coll r))) ) ->
              if CCList.subset ~eq:(fun a b -> a = b) l r
              then e_true
              else e_false
          | _l, _r ->
            ...
            Rec_value
              (Value
                 (Literal
                    (Datetime
                       (UTCDateOnly
                          (Datetime.convert_utctimestamp_utcdateonly t) ) ) ) )
    ...
    | Value (Funcall { func : string; args : record_item list })
      when func = "date" && args = [] ->
        Rec_value
          (Value
             (Literal
                (Datetime (UTCTimestamp (Datetime.get_current_utctimestamp ())))
             ) )
    | Value (Funcall { func : string; args : record_item list })
            ...
            Rec_value
              (Value
                 (Literal
                    (Datetime
                       (LocalMktDate
                          (Datetime.convert_utctimestamp_localmktdate t) ) ) )
              )
        | _ ->
            Rec_value e )
      | _ ->
  ...
  let rec infer_field_presence (context : 'a context) (e : expr) :
      field_path list =
    let evaluate_record_item = evaluate_record_item context in
    let evaluate_expr = evaluate_expr context in
    match e with
    | Cmp
        ...
        in
        if (is_ground def_val && is_ground ri)
  ...
  *)
  let nullable_expr
    ...
    in
    let this_msg_vars =
      get_all_subsets (Field_path_set.to_list this_msg_vars)
    in
    CCList.fold_left
      (fun acc el ->
        let expr =
        ...
        let record_item =
          fix_evaluate_record_item
            ...
            (Rec_value expr)
        in
        if is_true record_item then Field_path_set.of_list el :: acc else acc )
      []
      this_msg_vars
end
