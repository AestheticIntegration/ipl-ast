...
let comma_separated ppf = CCFormat.(list ~sep:(return ",@,") ppf)

let semi_separated ppf = CCFormat.(list ~sep:(return ";@,") ppf)

let rec expr_pp ppf = function
  | Paren x ->
  ...
  | Cond x ->
      fprintf
        ...
        x.orelse
  | StrLen x ->
      fprintf ppf "strLen(%a)" expr_pp x
  | IplNone ->
      fprintf ppf "None"
  | IplSome x ->
      fprintf ppf "Some(%a)" expr_pp x
  | OptCase { value; capture; some; none } ->
      fprintf
        ppf
        "case(%a){Some %s:%a}{Node: %a}"
        expr_pp
        value
        capture
        expr_pp
        ...
        none
  | ListLiteral x ->
      fprintf ppf "[@[<hov>%a@]]" (comma_separated expr_pp) x
  | SetLiteral x ->
      fprintf ppf "{|@[<hov>%a@]|}" (comma_separated expr_pp) x
  | MapLiteral { elements; default } ->
      fprintf
        ppf
        "<@[<hov>%a|default: %a@]>"
        (comma_separated map_element_pp)
        elements
        expr_pp
        default
  | ExternalFuncCall ->
  ...
  | Get { left; right } ->
      fprintf ppf "get(%a,%a)" expr_pp left expr_pp right
  | BoolConst x ->
      fprintf ppf (if x then "true" else "false")
  | IntConst x ->
      fprintf ppf "%d" x
  | FloatConst x ->
      fprintf ppf "%f" x
  | StringRef x ->
      fprintf ppf "\"%s\"" x
  | Field_vals x ->
      fprintf ppf "{@[<hov>%a@]}" (semi_separated field_val_pp) x
  | ToInt x ->
      fprintf ppf "toInt(%a)" expr_pp x
  | ToFloat x ->
      fprintf ppf "toFloat(%a)" expr_pp x
  | IntOfString x ->
      fprintf ppf "intOfString(%a)" expr_pp x
  | Present x ->
  ...
  | Exists (var, expr) ->
      fprintf ppf "exists(%a,%a)" expr_pp var lambda_expr_pp expr
  | Find (var, expr) ->
      fprintf ppf "find(%a,%a)" expr_pp var lambda_expr_pp expr
  | Filter (var, expr) ->
      fprintf ppf "filter(%a,%a)" expr_pp var lambda_expr_pp expr


and lambda_expr_pp ppf (x : lambda_expr) =
  fprintf
    ppf
    "{%a|%a}"
    CCFormat.(list ~sep:(return " ") string)
    x.vars
    expr_pp
    x.expr
...
let rec typedecl_pp ppf = function
  | Set x ->
      fprintf ppf "(%a) set" typedecl_pp x
  | List x ->
      fprintf ppf "(%a) list" typedecl_pp x
  | Option x ->
      fprintf ppf "(%a) option" typedecl_pp x
  | Map { map_key; map_value } ->
      fprintf ppf "<%a,%a> map" typedecl_pp map_key typedecl_pp map_value
  | Simple s ->
      fprintf ppf "%s" s
  | Element x ->
      fprintf ppf "(%a) element" typedecl_pp x
...
let rec statement_pp ppf = function
  ...
  | LetDecl { name; ltype; value } ->
    ( match ltype with
    | Some ltype ->
        fprintf ppf "let %s : %a = %a" name typedecl_pp ltype expr_pp value
    | None ->
        fprintf ppf "let %s = %a" name expr_pp value )
  | If { test; body; orelse } ->
    ( match orelse with
    | [] ->
        fprintf
    ...
    | _ ->
        fprintf
          ppf
          "if(%a) then {@;<1 2>@[<v>%a@]@,} else {@;<1 2>@[<v>%a@]@,}"
          expr_pp
          test
          statement_list_pp
          body
  ...
  | Insert { left; right; mapValue } ->
    ...
    | Some mapValue ->
        fprintf
          ...
          mapValue
    | None ->
        fprintf ppf "insert( %a , %a )" expr_pp left expr_pp right )
  ...
  | ReturnStatement { ret } ->
    ...
    | None ->
        fprintf ppf "return"
    | Some x ->
        fprintf ppf "return %a" expr_pp x )
...
let field_pp ppf (x : field) =
  fprintf
    ...
    "%s %s: %a"
    x.name
    (match x.tag with None -> "" | Some tag -> "\"" ^ tag ^ "\"")
    typedecl_pp
    x.ftype
...
let validator_pp ppf x = fprintf ppf "validate {%a}" expr_pp x
...
let case_list_pp ppf (x : case_decl list) =
  CCFormat.(
    list ~sep:(return "@,") (fun fmt { name; tag } ->
...
let typedArg_pp ppf (name, ttype) = fprintf ppf "%s:%a" name typedecl_pp ttype

let string_pp ppf name = fprintf ppf "%s" name
...
let req_type_pp = function REQ -> "req" | OPT -> "opt" | IGN -> "ign"
...
let require_list_pp ppf requires =
  CCFormat.(
    list ~sep:(return "@,") (fun fmt req -> fprintf fmt "%a" require_pp req))
    ppf
    requires
...
let attribute_pp ppf a =
  ...
  | _ ->
      fprintf
        ...
        "%s(%a)"
        a.name
        CCFormat.(list ~sep:(return " ,") attribute_type_pp)
        a.args
...
let using_list_pp ppf u =
  ...
  | _ ->
      fprintf
        ppf
        "using @,{@;<1 2>@[<v>%a@]@,}"
        CCFormat.(list ~sep:(return "@,") using_pp)
        u
...
let model_statement_pp ppf = function
  | Library s ->
  ...
  | Scenario { name; events } ->
      fprintf
        ppf
        "@[<v>messageFlows { %s {@;<1 2> @[<v>template [%a]@]@,}@]}"
        name
        CCFormat.(list ~sep:(return ",") string_pp)
        events
  | InternalDecl { name; assignable_fields; internal_fields } ->
      fprintf
        ppf
        "@[<v>internal %s {@;<1 2> %a @[<v>%a@]@,}@]"
        name
        assignable_pp
        assignable_fields
        internal_field_list_pp
        internal_fields
  | Receive { event; event_var; body } ->
      fprintf
        ppf
        "@[<v>receive ( %s : %s ) {@;<1 2>@[<v>%a@]@,}@]"
        event_var
        event
        statement_list_pp
        body
  | Reject
      ...
      fprintf
        ...
        missing_field
        (fun x y -> rejblock_pp x "invalid_field" y)
        invalid_field
        (fun x y -> rejblock_pp x "invalid" y)
        invalid
  | Function { name; args; returnType; body } ->
      fprintf
        ppf
        "@[<v>function %s(%a):%a {@;<1 2>@[<v>%a@]@,}@]"
        name
        CCFormat.(list ~sep:(return ",") typedArg_pp)
        args
        typedecl_pp
        returnType
        statement_list_pp
...
let program_pp ppf =
  fprintf
    ppf
    "@[<v>%a@]@."
    CCFormat.(list ~sep:(return "@,@,") model_statement_pp)
