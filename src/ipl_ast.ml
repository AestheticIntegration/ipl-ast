...
type expr =
  ...
  | IntConst of int
  | FloatConst of float
  | StringRef of string  (** Inplace record *)
  | Field_vals of field_val list  (** Converters *)
  | ToInt of expr
  | ToFloat of expr
  | IntOfString of expr
  | Present of expr
  ...
  | Find of expr * lambda_expr
  | Filter of expr * lambda_expr
...
type statement =
  ...
  | Assign of
      { lvalue : value_ref list
      ; expr : expr
      }
  | LetDecl of
      { name : string
      ; ltype : typedecl option
      ; value : expr
      }
  | If of
      { test : expr
...
type model_statement =
  ...
  | Enum of
      { name : string
      ; cases : case_decl list
      }
  | InternalDecl of
      { name : string
      ; assignable_fields : assignable_field list
      ; internal_fields : internal_field list
      }
  | Receive of
      { event : string
      ; event_var : string
